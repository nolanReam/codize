...
- Prompt Builder shows the real assignment before generic starters. `Use this assignment` selects it but does not prefill the Task field. M18C.1's explicit scope application writes the authoritative assignment plus the student's finish and inspection conditions into Task and the student's exclusion into Don't touch; Context and all other fields remain untouched.
...
- Local Prompt and scope drafts include the server-owned roadmap/assignment revision as well as phase, task id, and objective version. They participate in the same existing switch warning; no second confirmation is introduced, and a replacement roadmap cannot silently reuse an old task-id draft.
