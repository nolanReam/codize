...
- A Condition 3 reference must use the student's actual named element from the anchor or prior turns, verbatim — their table, column, policy, function, or file names. Generic role descriptions ("the owner column", "a junction table", "the authorization check") do not count, no matter how technically apt. Before returning a verdict, silently locate the exact substring of the Turn 3 response that names an anchor or prior-turn element; if no such substring exists, Condition 3 fails and the auto-fail rule applies.
- Describing only the modification itself does not satisfy Condition 2. The response must explicitly state at least one consequence for another part of their app — what else breaks, shifts, or must be updated as a result of the change. If no consequence beyond the modified component is stated, Condition 2 fails.
